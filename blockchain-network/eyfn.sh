...
function networkUp () {
  ...
  export BYFN_CA3_PRIVATE_KEY=$(cd org3-artifacts/crypto-config/peerOrganizations/org3.example.com/ca && ls *_sk)
  # start org3 peers
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
      IMAGE_TAG=${IMAGETAG} docker-compose -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_CA_ORG3  -f $COMPOSE_FILE_COUCH_ORG3 up -d 2>&1
  else
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_CA_ORG3 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
function networkDown () {
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_CA_ORG3 -f $COMPOSE_FILE_COUCH down --volumes --remove-orphans
...
COMPOSE_FILE_CA_ORG3=docker-compose-ca-org3.yaml
#
